...
import { CalendarIcon, Plus, ChevronLeft, ChevronRight, Clock, MapPin, Users, Edit, Trash2, Share2 } from "lucide-react"
...
export default function TeacherCalendarPage() {
  ...
  // Mock students for sharing events
  const mockStudents = [
    { id: "student1", name: "Trần Thị B", studentId: "SV001" },
    { id: "student2", name: "Lê Văn C", studentId: "SV002" },
    { id: "student3", name: "Nguyễn Thị D", studentId: "SV003" },
    { id: "student4", name: "Phạm Văn E", studentId: "SV004" },
  ]
  ...
  const saveEvent = async () => {
    ...
    try {
      const eventData = {
        ...eventForm,
        createdBy: user?.id,
        attendees: eventForm.type === "shared" || eventForm.type === "meeting" ? eventForm.attendees : undefined,
      }
  ...
  const toggleAttendee = (studentId: string) => {
    setEventForm((prev) => ({
      ...prev,
      attendees: prev.attendees.includes(studentId)
        ? prev.attendees.filter((id) => id !== studentId)
        : [...prev.attendees, studentId],
    }))
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Quản lý lịch trình và chia sẻ sự kiện với sinh viên</p>
        </div>
        <Dialog open={showEventDialog} onOpenChange={setShowEventDialog}>
          ...
          <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {/* Attendees selection for shared events */}
              {(eventForm.type === "shared" || eventForm.type === "meeting") && (
                <div>
                  <Label>Chia sẻ với sinh viên</Label>
                  <div className="mt-2 space-y-2 max-h-32 overflow-y-auto border rounded-md p-2">
                    {mockStudents.map((student) => (
                      <div key={student.id} className="flex items-center space-x-2">
                        <Checkbox
                          id={student.id}
                          checked={eventForm.attendees.includes(student.id)}
                          onCheckedChange={() => toggleAttendee(student.id)}
                        />
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Đã chọn {eventForm.attendees.length} sinh viên</p>
                </div>
              )}

              <div>
                <Label htmlFor="reminder">Nhắc nhở trước (phút)</Label>
                <Select
                  value={eventForm.reminder.toString()}
                  onValueChange={(value) => setEventForm({ ...eventForm, reminder: Number.parseInt(value) })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                    ...
                    >
                      ...
                      <div className="space-y-1 mt-1">
                        {dayEvents.slice(0, 2).map((event) => (
                          ...
                          >
                            {(event.type === "shared" || event.type === "meeting") && <Share2 className="h-2 w-2" />}
                            {event.title}
                          </div>
                        ))}
                        {dayEvents.length > 2 && (
                    ...
                    <div key={event.id} className="flex items-start gap-3 p-3 border rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-sm flex items-center gap-2">
                          {event.title}
                          {(event.type === "shared" || event.type === "meeting") && (
                        ...
                        {event.attendees && event.attendees.length > 0 && (
                          <div className="text-xs text-muted-foreground flex items-center gap-2 mt-1">
                            ...
                            {event.attendees.length} sinh viên
                          </div>
                        )}
              ...
              <div className="space-y-2">
                {Object.entries(eventTypeLabels).map(([type, label]) => (
                  <div key={type} className="flex items-center gap-3">
                    ...
                    {(type === "shared" || type === "meeting") && <Share2 className="h-3 w-3 text-blue-600" />}
                  </div>
                ))}
              </div>
            </CardContent>
